...
#### 6. 商品回仓
![order_back](img/order_back.png)
- 采用延迟队列+死信队列的方式实现订单超时回仓
- 在创建订单时，在延迟队列中投放当前订单消息，并设定超时时间TTL(15min)
- 在延迟队列中的某条消息超时后，就会被投放到死信队列中
- 订单服务监听死信队列，若死信队列中出现了新的消息，则对消息进行消费
  - 检查订单状态，若订单已支付，则不做其他处理
  - 若订单未支付，则进行回仓处理，并作废该订单
